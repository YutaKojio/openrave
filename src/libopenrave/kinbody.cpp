        ...
        FOREACHC(it, _vDOFOrderedJoints) {
            ...
            if( toadd > 0 ) {
                ...
                FOREACH(itj, _vDOFOrderedJoints) {
                    ss << (*itj)->GetName() << "(" << (*it)->GetDOFIndex() << "),  ";
                }
                ss << "];";
                throw OPENRAVE_EXCEPTION_FORMAT(_("env=%s, dof indices mismatch joint '%s' (dofindex=%d), toadd=%d, v.size()=%d in call GetDOFValues with %s"), GetEnv()->GetNameId()%(*it)->GetName()%(*it)->GetDOFIndex()%toadd%v.size()%ss.str(), ORE_InvalidState);
            }
            (*it)->GetValues(v,true);
        }
    }
